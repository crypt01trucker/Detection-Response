...
1. Open a new tab and go to the **Automation** tab in LimaCharlie.
2. Under **D&R Rules**, click on **Create Custom Rule**.
3. To simplify rule creation, find an existing rule that detects process creation and copy it as a template.
4. Modify this rule to detect LaZagne specifically.

Since LaZagne is a tool for credential harvesting, we’ll base our rule on detecting **NEW_PROCESS** events generated by LaZagne.

Below is an example of a D&R rule to detect LaZagne:
...
This rule specifies:
- **Event Types**: Only triggers if the event is a **NEW_PROCESS** or **EXISTING_PROCESS**.
- **Operating System**: Targets **Windows** systems only.
...
5. Copy this modified rule into your new custom rule in LimaCharlie.

6. Paste an example event of LaZagne to test if your rule correctly detects LaZagne.
